...
@test all([div(0x100000000,k)*k == Base.Random.RandIntGen(uint64(1:k)).u for k in 13 .+ int64(2).^(1:31)])
@test all([div(typemax(Uint64,k)*k == Base.Random.RandIntGen(uint64(1:k)).u for k in 13 .+ int64(2).^(32:62)])
@test all([div(0x100000000,k)*k == Base.Random.RandIntGen(int64(1:k)).u for k in 13 .+ int64(2).^(1:31)])
