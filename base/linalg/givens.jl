...
givens{T}(A::AbstractMatrix{T}  , i1::Integer, i2::Integer, col::Integer) = i1 < i2 ? Givens(size(A, 1), i1, i2, convert((T,T,T), givens(A[i1,col], A[i2,col]))...) : error("Second row index must be larger than the first")

*{T}(G1::Givens{T}, G2::Givens{T}) = Rotation(push!(push!(Givens{T}[], G2), G1))
*(R::Rotation, A::AbstractMatrix) = update!(R, copy(A))

A_mul_Bc(A::AbstractMatrix, R::Rotation) = update!(copy(A), R)
...
update!(G1::Givens, G2::Givens) = error("Operation not supported. Consider *")
function update!(G::Givens, A::AbstractMatrix)
	m, n = size(A)
	for i = 1:n
...
function update!(A::AbstractMatrix, G::Givens)
	m, n = size(A)
	for i = 1:m
...
function update!(G::Givens, R::Rotation)
	push!(R.rotations, G)
	return R
end
function update!(R::Rotation, A::AbstractMatrix)
	for i = 1:length(R.rotations)
		update!(R.rotations[i], A)
	end
	return A
end
function update!(A::AbstractMatrix, R::Rotation)
	for i = 1:length(R.rotations)
		update!(A, R.rotations[i])
	end
	return A
end
